...
class OneMessageMenu(Menu):
    ...
    def __init__(self) -> None:
        ...
        self.min_index: int = 1 if len(self._message) > 0 else 0
    ...
    def __repr__(self) -> str:
        ...
        author, quote = self._message[self.current_index]  # Returns a tuple (str, str)
        output: str = f"\n*{author}, dice: {quote}"
        return output

    def get_user_input(self, *args, **kwargs) -> int:
        self.current_index: int = get_user_input("mensaje", self.min_index, self.max_index) - 1
